...
/*
% Définition du prédicat qui choisit le meilleur coup parmi une liste de colonnes
meilleur_coup(_,_,[Colonne],Colonne) :- !. % Si il n'y a qu'une colonne, on la choisit
meilleur_coup(Grille,Poids,[C1,C2|Reste],Meilleur) :-
    poids(Grille,Poids,C1,P1), % On calcule le poids du premier coup
    poids(Grille,Poids,C2,P2), % On calcule le poids du deuxième coup
    (P1 > P2 -> MeilleurCoup = C1 ; MeilleurCoup = C2), % On compare les poids et on garde le meilleur coup
    meilleur_coup(Grille,Poids,[MeilleurCoup|Reste],Meilleur). % On continue avec le reste des colonnes
*/
...
prof_max(2). % On fixe la profondeur à 3
...
eval_plateau(Grille, Val) :-
    gagne(ia, Grille), % On vérifie si l'IA a gagné
    Val is 1000, % Si oui, on attribue une valeur très grande
    !. % On coupe pour éviter de chercher d'autres solutions
eval_plateau(Grille, Val) :-
    gagne(humain, Grille), % On vérifie si l'humain a gagné
    Val is -1000, % Si oui, on attribue une valeur très petite
    !. % On coupe pour éviter de chercher d'autres solutions
eval_plateau(Grille, Val) :-
    partie_nulle(Grille), % On vérifie si la partie est nulle
    ...
    !. % On coupe pour éviter de chercher d'autres solutions
eval_plateau(Grille, Val) :-
    totalSumscore(Grille, ia, ScoreIa), % On compte le nombre de pions de l'IA sur la grille
    totalSumscore(Grille, humain, ScoreH), % On compte le nombre de pions de l'humain sur la grille
    Val is ScoreIa*ScoreIa - ScoreH. % On calcule la différence entre les deux nombres
